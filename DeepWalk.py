...
class Model:
    def __init__(self, data_fold, save_fold):
        ...
        self.model = Word2Vec([[str(i)]for i in range(self.total_count)],
                              vector_size=self.embedding_size, window=3, min_count=1, sg=2)  # sg:1为skip-gram，2为CBOW
    ...
    def cal_exact_rate(self):
        ...
        first_exact_rate = 0
        for i in range(self.first_count):
            order = sorted(range(self.second_count),
                           key=lambda x: -self.model.wv.similarity(str(i), str(x+self.first_count)))
            for j in range(self.hits):
                if self.first_name[i] == self.second_name[order[j]]:
                    first_exact_rate += 1
        second_exact_rate = 0
        for i in range(self.second_count):
            order = sorted(range(self.first_count),
                           key=lambda x: -self.model.wv.similarity(str(i+self.first_count), str(x)))
            for j in range(self.hits):
